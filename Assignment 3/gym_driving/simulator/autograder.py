...
random_seeds_students = [13, 69, 420, 666, 42, 0, 2012, 7, 747, 8]
random_seeds_hidden = [11, 12, 23, 34, 45, 76, 65, 24, 99, 1]
task1_avg_students = [191.2, 134.1, 176.7, 152.5, 189.6, 138.5, 162.7, 169.4, 145.5, 162.1]
task2_avg_students = [239.3, 224.0, 257.0, 218.6, 218.4, 238.4, 298.4, 226.0, 183.7, 225.9]
task1_avg_hidden = [171.1, 156.7, 170.8, 169.4, 183.5, 162.1, 157.9, 178.8, 178.3, 176.7]
task2_avg_hidden = [227.2, 218.5, 242.3, 274.6, 258.0, 215.5, 279.2, 240.5, 219.6, 188.9]
flag_ok = 0

class VerifyOutputPlanner:
    def __init__(self, task, random_seeds, task1_avg, task2_avg):
        counter = 1
        ...
        for seed in random_seeds:
            ...
            # print("\n\n","-"*100)
            cmd_planner = "python", "run_simulator.py", "--task", task, "--random_seed", str(seed), "--frames_per_sec", str(500)
            ...
            roadFlag, time_success = self.verifyOutput(cmd_output, task, counter-1, task1_avg, task2_avg)
    ...
    def verifyOutput(self, cmd_output, task, counter, task1_avg, task2_avg):
...
if __name__ == "__main__":
    ...
    print('Student cases')
    algo = VerifyOutputPlanner('T1', random_seeds_students, task1_avg_students, task2_avg_students)
    algo = VerifyOutputPlanner('T2', random_seeds_students, task1_avg_students, task2_avg_students)
    print('Hidden cases')
    algo = VerifyOutputPlanner('T1', random_seeds_hidden, task1_avg_hidden, task2_avg_hidden)
    algo = VerifyOutputPlanner('T2', random_seeds_hidden, task1_avg_hidden, task2_avg_hidden)
